function fillMarketBorrowTable(tableId, listOfAssets, assetsToDisplay) {
  ...
  assetsToDisplay.forEach(function(assetAddress) {
    ...
    if (!asset.borrowEnabled) {
      return; //return in forEach loop is equivalent to continue in regular for loop
    }
    ...
    cell2.innerHTML = asset.stableDebtAPY.toFixed(2);
    if (asset.stableBorrowEnabled) {
...
function fillMarketSupplyTable(tableId, listOfAssets, assetsToDisplay) {
  // Get a reference to the table and tbody elements
  var table = document.getElementById(tableId);
  var tbody = table.getElementsByTagName("tbody")[0];
  ...
  assetsToDisplay.forEach(function(assetAddress) {
    ...
    if (asset.collateralEnabled) {
      ...
      cell4.innerHTML = "✘"
      newRow.className = "noCollateral"
    }
  })
}

function fillUserLendTable(tableId, listOfAssets, assetsToDisplay, ethPrice) {
  // Get a reference to the table and tbody elements
  var table = document.getElementById(tableId);
  var tbody = table.getElementsByTagName("tbody")[0];
  ...
  assetsToDisplay.forEach(function(assetAddress) {
    ...
    cell1.appendChild(img);
    cell1.innerHTML += "\t" + asset.name;
    cell2.innerHTML = asset.lendValue.toFixed(2);
    cell3.innerHTML = (asset.lendValue*asset.priceInETH*ethPrice).toFixed(2);
    cell4.innerHTML = asset.lendAPY.toFixed(2);
  })
}

function fillUserBorrowTable(tableId, listOfAssets, assetsToDisplay, ethPrice) {
  // Get a reference to the table and tbody elements
  var table = document.getElementById(tableId);
  var tbody = table.getElementsByTagName("tbody")[0];
  ...
  assetsToDisplay.forEach(function(assetAddress) {
    ...
    cell1.appendChild(img);
    cell1.innerHTML += "\t" + asset.name;
    if (asset.stableDebtValue != 0) {
      cell2.innerHTML = "Stable";
      cell3.innerHTML = asset.stableDebtValue.toFixed(2);
      cell4.innerHTML = (asset.stableDebtValue*asset.priceInETH*ethPrice).toFixed(2);
      cell5.innerHTML = asset.stableDebtAPY.toFixed(2);
    }
    else {
      cell2.innerHTML = "Variable";
      cell3.innerHTML = asset.variableDebtValue.toFixed(2);
      cell4.innerHTML = (asset.variableDebtValue*asset.priceInETH*ethPrice).toFixed(2);
      cell5.innerHTML = asset.variableDebtAPY.toFixed(2);
    }
