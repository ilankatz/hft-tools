async function getUserPositions(web3, lendPool, userAddress, listOfAssets, userBorrows, userLends) {
    ...
    const binaryList = binary.match(/.{1,2}/g).reverse()

    //Loop through all binary segments to see if user is borrowing or lending
    for (let i = 0; i < binaryList.length; i++) {
        if (binaryList[i][0] === "1") {
            ...
            userLends.push(reservesData[i])
            listOfAssets[reservesData[i]].lendValue = assetValue;
        }
        if (binaryList[i][1] === "1") {
            ...
            variableDebtTokenCon = new web3.eth.Contract(variableDebtTokenAbi, variableDebtTokenAddress)
            stableDebt = await stableDebtTokenCon.methods.balanceOf(userAddress).call() / 10**18
            variableDebt = await variableDebtTokenCon.methods.balanceOf(userAddress).call() / 10**18
            userBorrows.push(reservesData[i])
            listOfAssets[reservesData[i]].stableDebtValue = stableDebt;
            listOfAssets[reservesData[i]].variableDebtValue = variableDebt;
        }
    }
}
